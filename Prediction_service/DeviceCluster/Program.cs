...
public class Program
{
	...
	public static async Task Main(string[] args)
	{
		...
		try
		{
			...
			await sqlReader.QueryToJsonFileAsync("SELECT * FROM DummyInputOiltekA9995", SQL_OUTPUT_JSON);
			Console.WriteLine($"[Step 1/8] Reference data saved → {SQL_OUTPUT_JSON}\n");

			string requestJson = await sqlReader.QueryToJsonAsync(
				"SELECT ProjectCode, CustomerCode, DataIds FROM DummyInputOiltekA9995"
			);
			...
			if (request == null || request.data_ids == null || request.data_ids.Count == 0)
				throw new InvalidOperationException("No project data found in DummyInputOiltekA9995 table.");
