...
- **Device IDs are compared case-sensitively on the C# side.** Python normalises them, but `ClusterQuotaAllocator` (`HashSet<string>`) and `LogicAssignment` (`GroupBy`/`ToDictionary`) use the default ordinal comparer, so `V001.21` and `v001.21` are treated as two different devices. Same for `ProjectCode`, `Section` and `Cluster`. Worth fixing before the interface lets users type IDs by hand.
- **A tag prefix maps to exactly one device type, globally.** `extract_prefix_id` keeps only the leading letters, so `HT990` and `HT3137` are both `HT`. A manual correction overwrites that prefix for every device, past and future — the newest assignment wins. If a customer changes naming convention while old projects are still live, both meanings cannot coexist. The per-customer maps needed to scope this are already loaded (`customer_specific_map`) but are not used by the lookup.
- The ML service is registered as a Windows service and set to start at boot, but the server has not been restarted since, so boot-survival is configured rather than proven.
